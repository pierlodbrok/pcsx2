...
__fi void handleBadOp(mV, int count) {
	...
	if (mVUinfo.isBadOp) {
		...
		if (!isVU1) xFastCall((void*)mVUbadOp0, mVU.prog.cur->idx, xPC);
		else		xFastCall((void*)mVUbadOp1, mVU.prog.cur->idx, xPC);
...
void mVUdebugPrintBlocks(microVU& mVU, bool isEndPC) {
	if (mVUdebugNow) {
		mVUbackupRegs(mVU, true);
		if (isEndPC) xFastCall((void*)mVUprintPC2, xPC);
		else		 xFastCall((void*)mVUprintPC1, xPC);
		mVUrestoreRegs(mVU, true);
	}
}
...
void mVUtestCycles(microVU& mVU) {
	...
	if (doEarlyExit(mVU)) {
		...
		if (isVU0) {
			...
			// xFowardJZ32 vu0jmp;
			// mVUbackupRegs(mVU, true);
			// xFastCall((void*)mVUwarning0, mVU.prog.cur->idx, xPC); // VU0 is allowed early exit for COP2 Interlock Simulation
			// mVUrestoreRegs(mVU, true);
			...
			mVUbackupRegs(mVU, true);
			xFastCall((void*)mVUwarning1, mVU.prog.cur->idx, xPC);
			mVUrestoreRegs(mVU, true);
			mVUsavePipelineState(mVU);
			mVUendProgram(mVU, NULL, 0);
		}
